...
"""
Converts OpenURLs to BibJSON and back.
"""
...
class OpenURLParser(object):
    ...
    def parse(self):
        ...
        log.debug( 'type( d.items() ), `%s`' % type(d.items()) )
        log.debug( 'd.items(), ```%s```' % pprint.pformat(d.items()) )
        # for k,v in d.items():
        #     if not v:
        ...
        for k,v in d.items():
            if not v:
                ...
                new_d[k] = v
        #add the original openurl
        log.debug( 'new_d before being sent to BibJSONToOpenURL, ```%s```' % pprint.pformat(new_d) )
        new_d['_openurl'] = BibJSONToOpenURL(new_d).parse()
